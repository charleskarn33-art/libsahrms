...
- **Email:** Resend API (preferred), with a Gmail SMTP fallback for before a sending domain is verified — payslip delivery (Phase 4)
...
| `0015_leave_settings.sql` | Adds `leave_type_settings` (per-company defaults — entitlement days, max carry forward, paid/active — for each fixed `leave_type` enum value) and `leave_policies` (one row per company: leave year start, probation period, approval lead time, half-day/backdated/carry-forward/encashment toggles). Backs the Leave Settings page's Leave Types table and Policy Configuration form |
...
- Email delivery via `src/lib/mailer.ts`, a thin provider-agnostic wrapper over Resend (`src/lib/resend.ts`) and Gmail SMTP (`nodemailer`, `src/lib/email-templates.ts`): each employee is emailed only their own payslip PDF as an attachment; `payslip_deliveries.status` moves `queued → sent` or `queued → failed` with the error message stored for retry. **Provider selection is automatic** — Resend is used whenever `RESEND_API_KEY` is set, otherwise it falls back to Gmail SMTP if `GMAIL_USER`/`GMAIL_APP_PASSWORD` (a Gmail [App Password](https://myaccount.google.com/apppasswords), not the account password) are set. This lets a company start sending real payslip emails from a personal/company Gmail account immediately, then switch to Resend later just by adding `RESEND_API_KEY` — no code change, no redeploy of a different code path. Gmail's own sending limits apply (~500/day on a plain Gmail account, ~2,000/day on Google Workspace), and deliverability for bulk transactional mail is weaker than a dedicated provider like Resend, so it's meant as a bridge, not a long-term choice at scale.
...
- **`/leave/settings` (migration `0015`) was written and reviewed the same way, without a live database or browser** — same network constraint. `leave_type` stays the fixed 7-value enum from `0001`; the Leave Types table lets HR edit per-company *defaults* (entitlement days, carry-forward cap, paid/active) for each of those seven, it doesn't add arbitrary new leave types, since that would mean migrating `leave_balances`/`leave_requests` off the enum entirely. The Approval Workflow card intentionally shows the real single-step flow (Employee → HR/Admin) rather than the reference design's four-step chain, since a configurable multi-step approval chain doesn't exist in the schema yet — flag if that's wanted as a real feature.
